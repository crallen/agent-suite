...
Current review diff:
!`s=$(git diff --staged --name-only); u=$(git diff --name-only); o=$(git ls-files --others --exclude-standard); if [ -z "$s" ] && [ -z "$u" ] && [ -z "$o" ]; then printf '(no pending changes)'; else paths=$(printf '%s\n%s\n' "$( [ -n "$s" ] && printf '%s' "$s" || printf '%s' "$u" )" "$o" | sed '/^$/d'); if printf '%s\n' "$paths" | grep -E '(^|/)\.env(rc|\..*)?$|(^|/)\.npmrc$|(^|/)id_[^/]+$|credentials[^/]*\.json$|service-?account[^/]*\.json$|\.pem$|\.key$|\.p12$|\.pfx$|\.tfvars(\.json)?$|(^|/)secrets?[^/]*\.(ya?ml|json)$|(^|/)secrets/' >/dev/null 2>&1; then printf 'REVIEW BLOCKED: sensitive-looking files are present in the changes. Remove or redact them before using /code-review.\n\nChanged paths:\n%s\n' "$paths"; else { [ -n "$s" ] && git diff --staged || git diff; printf '%s\n' "$o" | sed '/^$/d' | while IFS= read -r f; do git diff --no-index /dev/null "$f" || true; done; }; fi; fi`
