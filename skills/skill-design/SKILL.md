...
- A **workflow skill** (the `/`-commands) is an action the human decides to take. It runs when the human names it, and preloading one makes no sense — it is a task prompt, not knowledge. It spends **cognitive load**: the human is the index that must remember it exists. This is the user-invoked kind.

Claude Code enforces the second kind with `disable-model-invocation: true`, which strips the description from the agent's reach and would cost zero context load. **This suite deliberately leaves that key off.** The block is absolute — the Skill tool refuses the skill and tells the agent not to reproduce the workflow by other means — so a client that mangles `/name` into `$name` on send leaves the command unreachable by either party. Workflow skills here keep an agent-facing description and pay context load like any reference skill; the human-only rule lives in `CLAUDE.md` as a convention instead. Read the two-load economics below as the general model, and remember this suite has already spent the context load on every skill it has.

Choose by what the skill *is*: knowledge the agent should consult mid-task is a reference skill, an action the human initiates is a workflow skill. Since neither kind escapes context load here, decide on purpose rather than cost — and prune a workflow skill's description as hard as a reference skill's.
